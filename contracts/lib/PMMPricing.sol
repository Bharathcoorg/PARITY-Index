...
/**
 * @title Pricing
 * @author DODO Breeder (adapted for Solidity 0.8.19)
 *
...
library PMMPricing {
    ...
    // ============ Query functions for pool integration ============

    function querySellBase(uint256 B, uint256 Q, uint256 amount, uint256 k, uint256 i) internal pure returns (uint256) {
        ...
        (uint256 receiveQuoteAmount, ) = sellBaseToken(state, amount);
        return receiveQuoteAmount;
    }

    function queryBuyBase(uint256 B, uint256 Q, uint256 amount, uint256 k, uint256 i) internal pure returns (uint256) {
        PMMState memory state = PMMState({
            i: i,
            K: k,
            B: B,
        ...
        (uint256 payQuoteAmount, ) = sellQuoteToken(state, amount);
